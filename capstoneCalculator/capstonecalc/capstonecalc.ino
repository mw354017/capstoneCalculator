  ...
  for(int i=0; i<=31; i++){
    if(totalInput[i] == '-') {
      ...
      numbers2[l].parenthesesDepth = parenthesesLevel;      
       parenthesesLevel -= rightParenthesesCounter;
       rightParenthesesCounter = 0;
      for(int z=0; z <32; z++) {
        numbers1[z] = {NULL};
      }
      l++;
      ...
      if(totalInput[i-1] == '+' ||  totalInput[i-1] == '_' || totalInput[i-1] == '*' || totalInput[i-1] == '/' || totalInput[i-1] == '^'){
        ...
        for(int n=0; n<=l; n++) {
          Serial.print(numbers2[n].realComponent);
          Serial.print(" The Parentheses depth: ");
          Serial.println(numbers2[n].parenthesesDepth);
        }
        l++; 
        m++;
        Serial.println(ops);
...
operand orderOfOps(operand numbers[32], char ops[31], int sizeNumbers, int sizeOps, int parenthesesLevel)
{
  ...
  if(numbers[leftNum].parenthesesDepth > 0) {
    numbers[leftNum].parenthesesDepth--;
        Serial.print(numbers[leftNum].realComponent);
        Serial.print(" The Parentheses depth: 666 ");
        Serial.println(numbers[leftNum].parenthesesDepth);
  }    
  return numbers[0];
}
